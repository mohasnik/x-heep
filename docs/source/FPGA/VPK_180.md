...
This README provides an introduction to Versal Prime device and step-by-step instructions for setting up, debugging, and programming the VPK180 FPGA board. It assumes that you have already generated the bitstream and installed the necessary tools. Please refer to the following before proceeding:
...
## Design Flow
The VPK180 target only supports **a** design flow with Processing System connections. Similar to **Processing System-enabled designs** on all other FPGA targets, in this flow X-HEEP is connected to **the** Processing System (PS) via JTAG, UART, and some of the GPIO pins.

Figure 1 shows the connections between X-HEEP and **the** PS. This **implementation** enables the APU to properly program applications **on** X-HEEP. Using JTAG, it can properly program the ELF file **into** the text section embedded memory banks, **select** the boot mode and **get** the return value of the application using **GPIOs**, and optionally **monitor** the stdout of the code being executed.
...
![Figure 1: ZCU104 board schematics highlighting flash interconnection and program switches.](./../images/VPK180-flow.drawio.png)
