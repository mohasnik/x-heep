...
begin
  ...
  always @(posedge fdsu_clk) begin
    ...
    else if (ctrl_srt_itering) srt_cnt[4:0] <= srt_cnt[4:0] - 5'b1;
  ...
  //==========================================================
  parameter WB_IDLE = 2'b00, WB_EX2 = 2'b10, WB_CMPLT = 2'b01;
...
begin
    case (wb_cur_state[1:0])
      ...
      if (fdsu_ex1_inst_vld)
        if (ctrl_xx_ex1_stall || fdsu_ex1_res_vld || fdsu_dn_stall) wb_nxt_state[1:0] = WB_IDLE;
  ...
  always @(posedge expnt_rst_clk) begin
    ...
    else if (ex1_pipedown) fdsu_expnt_rst[9:0] <= ex1_expnt_adder_op0[9:0];
    else if (ex2_pipedown) fdsu_expnt_rst[9:0] <= ex2_srt_expnt_rst[9:0];
    else if (ex3_pipedown) fdsu_expnt_rst[9:0] <= ex3_expnt_adjust_result[9:0];
