...
package dmi_test;
  ...
  class req_t #(
    ...
    function do_compare(req_t rhs);
      return addr == rhs.addr & op == rhs.op & data == rhs.data;
    endfunction
  ...
  class rsp_t;
    ...
    function do_compare(rsp_t rhs);
      return data == rhs.data & resp == rhs.resp;
    endfunction
  ...
  class dmi_driver #(
    ...
    virtual DMI_BUS_DV #(.ADDR_WIDTH(AW)) bus;
    ...
    task mon_req(output req_t req);
      cycle_start();
      while (!(bus.q_valid && bus.q_ready)) begin
        cycle_end();
        cycle_start();
      end
      req = new;
    ...
    task mon_rsp(output rsp_t rsp);
      cycle_start();
      while (!(bus.p_valid && bus.p_ready)) begin
        cycle_end();
        cycle_start();
      end
      rsp = new;
