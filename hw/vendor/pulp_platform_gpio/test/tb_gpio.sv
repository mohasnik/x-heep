...
program automatic test #(
  ...
  localparam type gpio_reg_driver_t = reg_driver#(
      .AW(AddrWidth),
      .DW(DataWidth),
      .TA(ApplTime),
      .TT(TestTime)
...
`endif
    ...
    assert (gpio_tx_en_i == '1)
    else begin
      $error(
          "GPIO TX driver not enabled although all GPIOs should be configured as outputs in push-pull mode.");
      error_count++;
    end
    assert (gpio_out_i == gpio_values[NrGPIOs-1:0])
    else begin
      $error("Missmatch in GPIO outputs. Expected output pattern %0b but was %0b.", gpio_values,
             gpio_out_i);
      error_count++;
    end
...
`endif
      ...
      for (int j = 0; j < NrGPIOs; j++) begin
        if (i == j) begin
          assert (gpio_out_i[j] == ~gpio_values[j])
          else begin
          ...
          assert (gpio_out_i[j] == gpio_values[j])
            ...
            $error(
                "GPIO %0d was %0b instead of %0b although it should not have beend altered during modification of GPIO %0d.",
                j, gpio_out_i[j], gpio_values[j], i);
            error_count++;
          end
        end
      end
...
`endif
        ...
        for (int j = i * 32; j < (i + 1) * 32; j++) begin
          if (j < NrGPIOs && enabled_gpios[j]) begin
            assert (gpio_in_o[j] == data[j%32])
            else begin
              $error("Got wrong gpio value for GPIO%0d. Was %0b instead of %0b", j, gpio_in_o[j],
                     data[j%32]);
              error_count++;
            end
          end
...
`endif
    ...
    for (int i = 0; i < NumRepetitions; i++) begin
      ...
      `SV_RAND_CHECK(randomize(toggle_mask) with {$countones(toggle_mask) < 3;});
      $info("Toggling GPIOs...");
      ...
      foreach (toggle_mask[j]) begin
        case (interrupt_modes[j])
          Falling: begin
            if (gpio_in_o[j] == 1'b0 && toggle_mask[j]) pending_fall_intrpt[j] = 1'b1;
          end

          Rising: begin
            if (gpio_in_o[j] == 1'b1 && toggle_mask[j]) pending_rise_intrpt[j] = 1'b1;
          end

          EitherEdge: begin
            if (toggle_mask[j]) begin
              if (gpio_in_o[j] == 1'b1) pending_rise_intrpt[j] = 1'b1;
              else pending_fall_intrpt[j] = 1'b1;
            end
          end

          Low: begin
            if (gpio_in_o[j] == 1'b0) pending_low_intrpt[j] = 1'b1;
          end

          High: begin
            if (gpio_in_o[j] == 1'b1) pending_high_intrpt[j] = 1'b1;
          end
        endcase
      end
      ...
      if (pending_intrpt) begin
        ...
        #TestTime;
        assert (global_interrupt_i == 1'b1)
        else begin
...
`endif
          gpio_reg_driver.send_read(addr, data, error);
          assert (data == pending_intrpt[i*32+:32])
          else begin
            $error("Interrupt status missmatch. Was %0x instead of %0x", data,
                   pending_intrpt[i*32+:32]);
            error_count++;
          end
`ifdef ENABLE_LESS_THAN_32_GPIOS_REG_PKG_WORKAROUND
...
`endif
          gpio_reg_driver.send_read(addr, data, error);
          assert (data == pending_rise_intrpt[i*32+:32])
          else begin
            $error("Interrupt rise status missmatch. Was %0x instead of %0x", data,
                   pending_rise_intrpt[i*32+:32]);
            error_count++;
          end
`ifdef ENABLE_LESS_THAN_32_GPIOS_REG_PKG_WORKAROUND
...
`endif
          gpio_reg_driver.send_read(addr, data, error);
          assert (data == pending_fall_intrpt[i*32+:32])
          else begin
            $error("Interrupt fall status missmatch. Was %0x instead of %0x", data,
                   pending_fall_intrpt[i*32+:32]);
            error_count++;
          end
`ifdef ENABLE_LESS_THAN_32_GPIOS_REG_PKG_WORKAROUND
...
`endif
          gpio_reg_driver.send_read(addr, data, error);
          assert (data == pending_low_intrpt[i*32+:32])
          else begin
            $error("Interrupt low status missmatch. Was %0x instead of %0x", data,
                   pending_low_intrpt[i*32+:32]);
            error_count++;
          end
`ifdef ENABLE_LESS_THAN_32_GPIOS_REG_PKG_WORKAROUND
...
`endif
          gpio_reg_driver.send_read(addr, data, error);
          assert (data == pending_high_intrpt[i*32+:32])
          else begin
            $error("Interrupt high status missmatch. Was %0x instead of %0x", data,
                   pending_high_intrpt[i*32+:32]);
            error_count++;
          end
        end
...
`endif
        ...
        assert (global_interrupt_i == 1'b0)
        else begin
...
`endif
    ...
    for (
        int repetition_idx = 0; repetition_idx < NumRepetitions; repetition_idx++
    ) begin : for_repetition
      // Set random gpio out values
      `SV_RAND_CHECK(randomize(gpio_values));
      for (int i = 0; i < (NrGPIOs + DataWidth - 1) / DataWidth; i++) begin
`ifdef ENABLE_LESS_THAN_32_GPIOS_REG_PKG_WORKAROUND
      ...
      for (int gpio_idx = 0; gpio_idx < NrGPIOs; gpio_idx++) begin : for_gpio
        unique case (gpio_modes[gpio_idx])
          2'b00: begin  // Input Only
            assert (gpio_tx_en_i[gpio_idx] == 1'b0)
            else begin
              $error("GPIO %d TX  is enabled even though GPIO is supposed to be in Input mode.",
                     gpio_idx);
              error_count++;
            end
          end
          ...
          2'b01: begin  // Push/Pull mode
            assert (gpio_tx_en_i[gpio_idx] == 1'b1)
            else begin
              $error(
                  "GPIO %d TX is not enabled even though GPIO is suppposed to be in Push/Pull mode.",
                  gpio_idx);
              error_count++;
            end
            assert (gpio_out_i[gpio_idx] == gpio_values[gpio_idx])
            else begin
              $error("Got wrong output value on GPIO %d. Was suposed to be %b but was %b.",
                     gpio_idx, gpio_values[gpio_idx], gpio_out_i[gpio_idx]);
              error_count++;
            end
          end
          ...
          2'b10: begin  // Open Drain mode 0, Drive high on value=1
            if (gpio_values[gpio_idx] == 1'b1) begin
              assert (gpio_out_i[gpio_idx] == 1'b1)
              else begin
                $error("Got wrong output value on GPIO %d. Was suposed to be %b but was %b.",
                       gpio_idx, gpio_values[gpio_idx], gpio_out_i[gpio_idx]);
                error_count++;
              ...
              assert (gpio_tx_en_i[gpio_idx] == 1'b1)
                ...
                $error(
                    "GPIO %d TX enable is not high even thought the GPIO is in Open Drain Mode 0 and the output is driven high.",
                    gpio_idx);
                error_count++;
              end
            end else begin
              assert (gpio_tx_en_i[gpio_idx] == 1'b0)
                ...
                $error(
                    "GPIO %d TX enable is asserted even thought the GPIO is in Open Drain Mode 0 and the output is driven low.",
                    gpio_idx);
                error_count++;
              end
            end
          ...
          2'b11: begin  // Open Drain mode 1, Drive low on value=0
            if (gpio_values[gpio_idx] == 1'b0) begin
              assert (gpio_out_i[gpio_idx] == 1'b0)
              else begin
                $error("Got wrong output value on GPIO %d. Was suposed to be %b but was %b.",
                       gpio_idx, gpio_values[gpio_idx], gpio_out_i[gpio_idx]);
                error_count++;
              ...
              assert (gpio_tx_en_i[gpio_idx] == 1'b1)
                ...
                $error(
                    "GPIO %d TX enable is not high even thought the GPIO is in Open Drain Mode 1 and the output is driven low.",
                    gpio_idx);
                error_count++;
              end
            end else begin
              assert (gpio_tx_en_i[gpio_idx] == 1'b0)
                ...
                $error(
                    "GPIO %d TX enable is asserted even thought the GPIO is in Open Drain Mode 1 and the output is driven high.",
                    gpio_idx);
                error_count++;
              end
            end
