...
package axi_test;
  ...
  class axi_rand_master #(
    ...
    function void add_traffic_shaping(input int unsigned len, input int unsigned freq);
      if (traffic_shape.size() == 0) traffic_shape.push_back({len, freq});
      else traffic_shape.push_back({len, traffic_shape[$].cprob + freq});
    ...
    task send_ws(ref logic aw_done);
      while (!(aw_done && w_queue.size() == 0)) begin
        ...
        for (int unsigned i = 0; i < aw_beat.ax_len + 1; i++) begin
          ...
          drv.send_w(w_beat);
          if (aw_beat.ax_burst == axi_pkg::BURST_INCR) addr += n_bytes;
        end
      end
    endtask
  ...
  class axi_lite_rand_master #(
    ...
    task automatic write(input addr_t w_addr, input prot_t w_prot = prot_t'(0), input data_t w_data,
      ...
      $display("%0t %s> Write to ADDR: %h, PROT: %b DATA: %h, STRB: %h", $time(), this.name,
               w_addr, w_prot, w_data, w_strb);
      fork
        this.drv.send_aw(w_addr, w_prot);
        this.drv.send_w(w_data, w_strb);
      join
      this.drv.recv_b(b_resp);
      $display("%0t %s> Received write response from ADDR: %h RESP: %h", $time(), this.name,
               w_addr, b_resp);
    endtask : write
    ...
    task automatic read(input addr_t r_addr, input prot_t r_prot = prot_t'(0), output data_t r_data,
      ...
      $display("%0t %s> Read from ADDR: %h PROT: %b", $time(), this.name, r_addr, r_prot);
      this.drv.send_ar(r_addr, r_prot);
      this.drv.recv_r(r_data, r_resp);
      $display("%0t %s> Recieved read response from ADDR: %h DATA: %h RESP: %h", $time(),
               this.name, r_addr, r_data, r_resp);
    endtask : read
  endclass
    ...
    protected task automatic handle_write_resp(input axi_id_t id);
      ...
      forever begin
        ...
        if (check_en[BRespCheck]) begin
          ...
          else $warning("Behavior for b_resp != axi_pkg::RESP_OKAY not modeled.");
        end
