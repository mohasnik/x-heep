...
class rv_timer_ctrl_reg_cov_obj extends uvm_object;
  ...
  covergroup timer_active_cg(string name) with function sample (bit [31:0] value);
    cp_active_timers: coverpoint value {bins all_timers_active[] = {{NUM_TIMERS{1'b1}}};}
...
class rv_timer_env_cov extends cip_base_env_cov #(
  ...
  function new(string name, uvm_component parent);
    ...
    foreach (rv_timer_prescale_values_cov_obj[timer, bit_num]) begin
      rv_timer_prescale_values_cov_obj[timer][bit_num] =
          new($sformatf("rv_timer-%0d-prescale-%0d", timer, bit_num));
    end
