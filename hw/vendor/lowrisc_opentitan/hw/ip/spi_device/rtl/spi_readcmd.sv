...
#(
  ...
  always_comb begin
    ...
    if (addr_4b_en_i) begin
      ...
      addr_ready_in_word = (s2p_bitcnt_i == BitCntW'(8 + 24 + 6)) ? 1'b1 : 1'b0;
    end else begin
      // 3B
      addr_ready_in_word = (s2p_bitcnt_i == BitCntW'(8 + 16 + 6)) ? 1'b1 : 1'b0;
    end
  end
  ...
  always_comb begin
    ...
    if (addr_4b_en_i) begin
      addr_latched = (s2p_bitcnt_i == BitCntW'(8 + 32)) ? 1'b1 : 1'b0;
    end else begin
      addr_latched = (s2p_bitcnt_i == BitCntW'(8 + 24)) ? 1'b1 : 1'b0;
    end
  end
  ...
  assign sram_we_o = 1'b0;  // always read
  assign sram_wdata_o = '0;  // always read

  //- END:   SRAM Datapath ----------------------------------------------------

  //= BEGIN: FIFO to P2S datapath =============================================
  assign fifo_byte = '{fifo_rdata[7:0], fifo_rdata[15:8], fifo_rdata[23:16], fifo_rdata[31:24]};
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      fifo_byteoffset <= fifo_byteoffset + 1'b1;
    end
  end
  ...
  always_comb begin
    ...
    unique case (main_st)
      ...
      MainOutput: begin
        ...
        if (bitcnt == 5'h2) begin
          addr_inc = 1'b1;
          sram_req = 1'b1;
        end

        if (bitcnt == 5'h0) begin
          // sent all words
          bitcnt_update = 1'b1;
          // TODO: FIFO pop here?
          fifo_pop = 1'b1;
