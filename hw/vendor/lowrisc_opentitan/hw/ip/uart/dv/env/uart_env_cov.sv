...
class uart_env_cov extends cip_base_env_cov #(
    .CFG_T(uart_env_cfg)
);
  ...
  covergroup fifo_level_cg with function sample (uart_dir_e dir, int lvl, bit rst);
    ...
    cp_lvl: coverpoint lvl {bins all_levels[] = {[0 : UART_FIFO_DEPTH]};}
    cp_rst: coverpoint rst;
    cross cp_dir, cp_lvl, cp_rst;
  endgroup

  // Cover all combinations of 2 different clocks
  covergroup baud_rate_w_core_clk_cg with function sample (
      baud_rate_e baud_rate, clk_freq_mhz_e clk_freq
  );
