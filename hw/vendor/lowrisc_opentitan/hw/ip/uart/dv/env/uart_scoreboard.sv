...
class uart_scoreboard extends cip_base_scoreboard #(
  ...
  virtual function void predict_tx_watermark_intr(uint tx_q_size = tx_q.size);
    uint watermark = get_watermark_bytes_by_level(
        ral.fifo_ctrl.txilvl.get_mirrored_value(), UartTx
    );
    intr_exp[TxWatermark] = get_non_sticky_interrupt(
        .cur_intr(intr_exp[TxWatermark]),
  ...
  virtual function void predict_rx_watermark_intr(uint rx_q_size = rx_q.size);
    uint watermark = get_watermark_bytes_by_level(
        ral.fifo_ctrl.rxilvl.get_mirrored_value(), UartRx
    );
          ...
          if (txrst_val) begin
            ...
            if (cfg.en_cov) begin
              cov.fifo_level_cg.sample(.dir(UartTx),
                                       .lvl(ral.fifo_status.txlvl.get_mirrored_value()), .rst(1));
            end
          end
          if (rxrst_val) begin
            rx_q.delete();
            ...
            if (cfg.en_cov) begin
              cov.fifo_level_cg.sample(.dir(UartRx),
                                       .lvl(ral.fifo_status.rxlvl.get_mirrored_value()), .rst(1));
            end
          end
          ...
          case (channel)
            ...
            DataChannel: begin  // check at data phase
              ...
              if (!(tx_enabled && tx_q_size_at_addr_phase inside {0, 1} && is_in_ignored_period(
                ...
                `DV_CHECK_EQ(
                    get_field_val(ral.status.txempty, item.d_data), tx_empty_exp,
              ...
              if (!(rx_q_size_at_addr_phase inside {0, 1} && is_in_ignored_period(UartRx))) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.rxempty, item.d_data), rx_empty_exp,
              ...
              if (!(tx_q_size_at_addr_phase == 0 && is_in_ignored_period(UartTx))) begin
                `DV_CHECK_EQ(
                    get_field_val(ral.status.txidle, item.d_data), tx_idle_exp,
                        $sformatf(
              ...
              if (!(uart_rx_clk_pulses inside {1, uart_bits}) || !rx_enabled) begin
                `DV_CHECK_EQ(get_field_val(ral.status.rxidle, item.d_data), rx_idle_exp,
                                 $sformatf(
        ...
        if (write && channel == AddrChannel) begin  // write & address phase
          ...
          foreach (intr_exp[i]) begin
            ...
            if (intr inside {TxWatermark, TxEmpty}) begin  // TX interrupts
              ...
              if (intr inside {RxTimeout, RxBreakErr} || is_in_ignored_period(
                      UartRx
            ...
            `DV_CHECK_EQ(item.d_data[i], intr_exp_at_addr_phase[i], $sformatf(
                         "Interrupt: %0s", intr.name));
          ...
          case (channel)
            ...
            DataChannel: begin  // check at data phase
              ...
              if (txlvl_act == txlvl_exp) begin
                // match, do nothing
              end else if (tx_enabled && is_in_ignored_period(
                      UartTx
                  ) && absolute(
                      txlvl_act - txlvl_exp
                  ) == 1) begin
              ...
              if (rxlvl_act == rxlvl_exp) begin
                // match, do nothing
              end else if (rx_enabled && is_in_ignored_period(
                      UartRx
                  ) && absolute(
                      rxlvl_act - rxlvl_exp
                  ) == 1) begin
  ...
  function void compare(uart_item act, uart_item exp, string dir = "TX");
    if (!act.compare(exp)) begin
      ...
      `uvm_info(`gfn, $sformatf(
                "%s item match!\nexp:\n%0s\nobs:\n%0s", dir, exp.sprint(), act.sprint()), UVM_HIGH)
    end
  endfunction
