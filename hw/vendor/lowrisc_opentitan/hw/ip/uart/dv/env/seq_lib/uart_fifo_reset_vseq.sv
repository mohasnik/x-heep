...
class uart_fifo_reset_vseq extends uart_fifo_overflow_vseq;
  ...
  virtual task process_remaining_data();
    ...
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(do_clear_tx_fifo, do_clear_tx_fifo dist {0 :/ 1,
                                                                                1 :/ 4};)
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(do_clear_rx_fifo, do_clear_rx_fifo dist {0 :/ 1,
                                                                                1 :/ 4};)
