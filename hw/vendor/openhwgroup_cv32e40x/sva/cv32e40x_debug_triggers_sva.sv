...
#(
  ...
  assert property (@(posedge clk) disable iff (!rst_n)
                  ...
                  $stable(
      tdata1_q
  ))  // Checking stability of ALL tdata1, not just the one selected
  ...
  assert property (@(posedge clk) disable iff (!rst_n)
                  ...
                  $stable(
      tdata2_q
  ))  // Checking stability of ALL tdata2, not just the one selected
      ...
      for (genvar idx = 0; idx < DBG_NUM_TRIGGERS; idx++) begin
        ...
        lsu_valid_ex_i
          ...
          `uvm_error("debug_triggers",
                     "Address matching not enabled on an AMO instruction when trig on loads are configured")
      end
    end
  endgenerate
