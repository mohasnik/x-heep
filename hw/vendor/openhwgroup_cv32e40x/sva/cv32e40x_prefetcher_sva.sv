...
#(
  ...
  else `uvm_error("Prefetcher SVA", $sformatf("trans_valid_o active when fetch_valid_i is not"))
  ...
  assert property (p_branch_addr)
    ...
    `uvm_error("Prefetcher SVA", $sformatf("branch address not propagated to trans_addr_o correctly"
               ))
  ...
  else `uvm_error("Prefetcher SVA", $sformatf("fetch_branch_addr_i is not halfword aligned."))
  ...
  else `uvm_error("Prefetcher SVA", $sformatf("trans_addr_o is not halfword aligned."))
  ...
  assert property (p_fetch_ready)
    ...
    `uvm_error("Prefetcher SVA", $sformatf(
               "fetch_ready_o not set when trans_ready_i && trans_valid_o."))
  ...
  else `uvm_error("Prefetcher SVA", $sformatf("Address increment not 4."))
  ...
  else `uvm_error("Prefetcher SVA", $sformatf("First fetch after reset is not a branch"))
  ...
  if (CLIC) begin
    ...
    else `uvm_error("Prefetcher SVA", "data_q is set on branch.")
  end  // CLIC
