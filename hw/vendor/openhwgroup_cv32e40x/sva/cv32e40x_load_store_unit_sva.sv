...
#(
  ...
  assert property (@(posedge clk) disable iff (!rst_n) (cnt_q != 0) |-> busy_o)
  ...
  always_comb begin
    ...
    if (m_c_obi_data_if.s_req.req && m_c_obi_data_if.s_gnt.gnt) outstanding_cnt++;
    if (m_c_obi_data_if.s_rvalid.rvalid) outstanding_cnt--;
  end
  ...
  assert property (@(posedge clk) disable iff (!rst_n) (outstanding_cnt_q <= 2))
  ...
  else `uvm_error("load_store_unit", "Assertion a_no_spurious_rvalid failed")
  ...
  assert property (@(posedge clk) disable iff (!rst_n) !X_EXT |-> !xif_req)
  else `uvm_error("load_store_unit", "XIF transaction request despite X_EXT being disabled")
  a_lsu_no_xif_res_if_xext_disabled :
  assert property (@(posedge clk) disable iff (!rst_n) !X_EXT |-> !xif_res_q)
  ...
  a_stable_tmatch :
  assert property (@(posedge clk) disable iff (!rst_n) (valid_0_i && !ready_0_i) |=> $stable(
      trigger_match_0_i
  ) until_with (ready_0_i))
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (ctrl_fsm_i.kill_ex) begin
        ...
        split_rem_cnt <= 2'h0;
      end else if (lsu_split_0_o && !(|split_rem_cnt)) begin
        // New split transfer
      ...
      end else if (m_c_obi_data_if.s_req.req && m_c_obi_data_if.s_gnt.gnt) begin
  ...
  if (DEBUG) begin
    ...
    assert property (@(posedge clk) disable iff (!rst_n)
      ...
      `uvm_error("load_store_unit",
                 "Illegal cause of cnt_q=0 while a valid LSU instruction is in WB")
  ...
  if (A_EXT != A_NONE) begin
    ...
    assert property (@(posedge clk) disable iff (!rst_n) trans_valid &&  // Active transfer
    (trans.atop[5] && (trans.addr[1:0] != 2'b00)) &&  // Misaligned atomic
    !mpu_err_i  // Not blocked by MPU
    |-> lsu_split_0_o && align_err_i && !bus_trans_valid)
