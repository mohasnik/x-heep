...
#(
  ...
  a_instr_addr_aligned :
  assert property (p_instr_addr_aligned)
  ...
  if (CLIC) begin
    ...
    assert property (@(posedge clk) disable iff (!rst_n)
      ...
      `uvm_error("if_stage", "prefetch_is_mret_ptr high at the same time as prefetch_is_clic_ptr.")
    ...
    assert property (@(posedge clk) disable iff (!rst_n) align_err_i &&  // Alignment error flagged
    ...
    a_no_align_err :
    assert property (@(posedge clk) disable iff (!rst_n) !align_err_i)
