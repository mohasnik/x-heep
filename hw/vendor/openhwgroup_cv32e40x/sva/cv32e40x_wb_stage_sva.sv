...
#(
  ...
  assert property (@(posedge clk) disable iff (!rst_n) (lsu_mpu_status_q == MPU_OK))
  ...
  assert property (@(posedge clk) disable iff (!rst_n)
    ...
    `uvm_error("wb_stage",
               "wb_valid not signaled immediately upon rvalid for LSU without WPT or MPU error")

  generate
    if (DEBUG) begin
      // The only cause for a sticky lsu_valid is a watchpoint trigger.
      a_only_wpt_lsu_valid_sticky :
      assert property (@(posedge clk) disable iff (!rst_n) lsu_valid_q |-> lsu_wpt_match_q && $past(
          lsu_wpt_match_i
      ))
      ...
      a_wpt_wb_valid :
      assert property (@(posedge clk) disable iff (!rst_n) lsu_wpt_match_i |=> wb_valid)
      ...
      assert property (@(posedge clk) disable iff (!rst_n) !lsu_valid_q)
