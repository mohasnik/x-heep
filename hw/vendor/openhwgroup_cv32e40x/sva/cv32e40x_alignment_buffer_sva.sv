...
(
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("FIFO overflow"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("Fifo not cleared after branch"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("Fifo not empty during branch cycle"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("instr_valid_o active when branch_i=1"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("trans_valid_o active when not supposed to"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("resp_valid_gated active when not supposed to"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("fetch_branch_addr_o not correctly set"))
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (ctrl_fsm_i.pc_set) begin
        branch_check <= 1'b1;
      end else if (instr_valid_o) begin
        branch_check <= 1'b0;
      end
    end
  end
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("Wrong PC after branch"))
  ...
  assert property (p_max_outstanding)
    ...
    `uvm_error("Alignment buffer SVA", $sformatf("Number of outstanding transactions exceeds 2!"))
  ...
  else `uvm_error("Alignment buffer SVA", $sformatf("Illegal resp_i.mpu_status"))
  ...
  else `uvm_error("Alignment buffer SVA", "Write pointer illegal value")
  ...
  else `uvm_error("Alignment buffer SVA", "Read pointer illegal value")
  ...
  else `uvm_error("Alignment buffer SVA", "Read pointer(2) illegal value")

  // Check that the alignment buffer is empty when a pointer is received.
  property p_pointer_fetch_empty;
    @(posedge clk) disable iff (!rst_n) ptr_fetch_accepted_q && resp_valid_i |-> !(|valid_q);
  endproperty

  a_pointer_fetch_empty :
  assert property (p_pointer_fetch_empty)
  else `uvm_error("Alignment buffer SVA", "Buffer not empty when requested pointer arrives.")
  ...
  else `uvm_error("Alignment buffer SVA", "Multiple instr_valid_o for one pointer")
  ...
  property p_no_outstanding_instr_valid;
    @(posedge clk) disable iff (!rst_n) ctrl_fsm_i.instr_req &&  // We are commanded to prefetch
    ...
    or ptr_flagged;  // Or a pointer has already been sent out
  endproperty

  a_no_outstanding_instr_valid :
  assert property (p_no_outstanding_instr_valid)
  else `uvm_error("Alignment buffer SVA", "No instr_valid when expected")
