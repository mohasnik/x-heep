...
#(
  ...
  if (CLIC) begin
    ...
    a_mtvec_mode_clic :
    assert property (@(posedge clk) disable iff (!rst_n) 1'b1 |-> mtvec_mode_o == 2'b11)
    ...
    a_no_mnxti_after_lsu :
    assert property (p_no_mnxti_after_lsu)
    ...
    a_htrap_interrupt_level :
    assert property (p_htrap_interrupt_level)
    ...
    a_mscratchcsw_mscratch :
    assert property (p_mscratchcsw_mscratch)
    ...
    a_mscratchcswl_mscratch :
    assert property (p_mscratchcswl_mscratch)
    ...
    a_mscratchcswl_mscratch_we :
    assert property (p_mscratchcswl_mscratch_we)
    else `uvm_error("cs_registers", "Mscratch not written by mscratchcswl");

    property p_mstatus_mcause_we;
      @(posedge clk) disable iff (!rst_n) (1'b1 |-> mstatus_we == mcause_we);
    endproperty
    ;
    a_mstatus_mcause_we :
    assert property (p_mstatus_mcause_we)
    ...
    endproperty
    ;
    a_mcause_mstatus_alias :
    assert property (p_mcause_mstatus_alias)
  ...
  a_halt_limited_wb :
  assert property (p_halt_limited_wb)
  ...
  a_minhv_unique :
  assert property (p_minhv_unique)
  ...
  generate
    if (DEBUG) begin
      ...
      a_etrigger_mepc_write :
      assert property (p_etrigger_mepc_write)
      else
        `uvm_error("cs_registers", "mepc not written with ctrl_fsm.pipe_pc when etrigger fires.");
      ...
      property p_etrigger_dpc_write;
        ...
        @(posedge clk) disable iff (!rst_n)
      (  ((ctrl_fsm_cs == FUNCTIONAL) && etrigger_wb_o && !(ctrl_fsm_i.halt_wb || ctrl_fsm_i.kill_wb),
        mtvec_at_trap = mtvec_addr_o
        ) |=> (!wb_valid_i && !ex_wb_pipe_i.instr_valid && dpc_we &&
               (dpc_n == {mtvec_at_trap, {7'd0}}) && (ctrl_fsm_cs == DEBUG_TAKEN)));
      endproperty
      ;

      a_etrigger_dpc_write :
      assert property (p_etrigger_dpc_write)
      else
        `uvm_error("cs_registers",
                   "dpc not written with first handler instruction when etrigger fires.");
