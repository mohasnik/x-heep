...
module rvb_bitcnt #(
  ...
  wire [XLEN-1:0] sextval = {
    {XLEN - 16{sextbit}}, din_insn20 ? din_rs1[15:8] : {8{din_rs1[7]}}, din_rs1[7:0]
  };
  ...
  always @* begin
    ...
    if (!revmode) data = din_rs1;
    if (czmode) data = (data - 1) & ~data;
    if (wmode) data = data & 32'hFFFFFFFF;

    cnt = 0;
    for (i = 0; i < XLEN; i = i + 1) cnt = cnt + data[i];

    for (i = 0; i < XLEN; i = i + 1) transp[i] = din_rs1[{i[2:0], i[5:3]}%XLEN];
  end
