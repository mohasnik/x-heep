...
module rvb_shifter #(
  ...
  wire [31:0] bfp_mask = 32'hFFFFFFFF << bfp_len;
  ...
  always @* begin
    ...
    if (wmode || !din_insn26) shamt[6] = 0;

    if (wmode && !din_insn26) shamt[5] = 0;

    if (din_insn14) shamt = -shamt;

    if (!din_insn26) begin
      casez ({
        din_insn30, din_insn29
      })
        2'b0z: bb = {64{din_insn29}};
        2'b10: bb = {64{wmode ? A[31] : A[63]}};
        2'b11: bb = A;
      endcase
  ...
  always @* begin
    ...
    if (sbmode) begin
      casez ({
        din_insn30, din_insn29, din_insn14
      })
        3'bzz1: Y = 1 & X;
        3'b0zz: Y = A | X;
        3'bz0z: Y = A & ~X;
    ...
    if (bfpmode) Y = (A & ~X) | {32'b0, din_rs2[31:0] & ~bfp_mask} << bfp_off;
  end
