...
module bfly_net #(
    ...
    for (genvar r = 0; unsigned'(r) < NumRouters; r++) begin : gen_routers2
      ...
      if (l == 0 && NeedsR2Stage) begin : gen_r4r2_level
        ...
        for (genvar k = 0; unsigned'(k) < Radix; k++) begin : gen_map
          ...
          assign data_in[l][r][k] = {
            router_add_in[l][r][k] << $clog2(Radix), router_data_in[l][r][k]
          };
          assign {router_add_out[l][r][k], router_data_out[l][r][k]} = data_out[l][r][k];
          ...
          if (BankFact < Radix) begin : gen_reverse
            for (genvar j = 0; unsigned'(j) < $clog2(Radix); j++) begin : gen_connect
              assign prio[l][r][k][$clog2(
                  Radix
              )-1-j] = rr[$clog2(
                  NumOut
              )-(unsigned'(l)+1-32'(NeedsR2Stage))*$clog2(
                  Radix
              )-32'(NeedsR2Stage)+unsigned'(j)];
            end
          end else begin : gen_no_reverse
            for (genvar j = 0; unsigned'(j) < $clog2(Radix); j++) begin : gen_connect
              assign prio[l][r][k][j] = rr[$clog2(
                  NumOut
              )-(unsigned'(l)+1-32'(NeedsR2Stage))*$clog2(
                  Radix
              )-32'(NeedsR2Stage)+unsigned'(j)];
  ...
  initial begin
    ...
    assert (Radix inside {2, 4})
    else $fatal(1, "Only Radix-2 and Radix-4 is supported.");
    ...
    assert (NumOut >= NumIn)
    else $fatal(1, "NumOut < NumIn is not supported.");
  end
  // pragma translate_on
