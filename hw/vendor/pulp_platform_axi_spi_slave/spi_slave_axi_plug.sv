...
module spi_slave_axi_plug #(
  ...
  always_ff @(posedge axi_aclk or negedge axi_aresetn) begin
    if (axi_aresetn == 0) begin
      ...
      if (sample_axidata) curr_data_tx <= axi_master_r_data;
  ...
  always_ff @(posedge axi_aclk or negedge axi_aresetn) begin
    if (axi_aresetn == 1'b0) tx_counter <= 16'h0;
    else if (start_tx) tx_counter <= 16'h0;
    else if (incr_addr_w | incr_addr_r) begin
      if (tx_counter == wrap_length - 1) tx_counter <= 16'h0;
  ...
  always_comb begin
    ...
    else if (tx_counter == wrap_length - 1) next_addr = rxtx_addr;
  ...
  always_comb begin
    ...
    unique case (aw_q)
      ...
      AxiResp: begin
        ...
        if (axi_master_b_valid) aw_d = Idle;
        else aw_d = AxiResp;
      end
    ...
    case (ar_q)
      ...
      Data: begin
        ...
        if (cs) begin
          ...
          if (tx_ready)
            if (tx_counter == wrap_length - 1) begin
              ar_d = Idle;
            end else begin
              incr_addr_r = 1'b1;
              ar_d        = AxiAddr;
      ...
      AxiAddr: begin
        ...
        if (axi_master_ar_ready) ar_d = AxiResp;
        else ar_d = AxiAddr;
      end
      AxiResp: begin
  ...
  generate
    if (AXI_DATA_WIDTH == 32) assign tx_data = curr_data_tx[31:0];
    else assign tx_data = curr_addr[2] ? curr_data_tx[63:32] : curr_data_tx[31:0];
  endgenerate
  ...
  generate
    if (AXI_DATA_WIDTH == 32) assign axi_master_w_strb = 4'hF;
    else assign axi_master_w_strb = curr_addr[2] ? 8'hF0 : 8'h0F;
  endgenerate
