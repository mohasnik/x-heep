...
module cv32e40p_prefetch_controller_sva #(
  ...
  assert property (p_no_transaction_count_overflow_0)
    ...
    `uvm_error("Prefetch Controller SVA", $sformatf(
               "Outstanding transaction count (%0d) greater than DEPTH (%0d)", cnt_q, DEPTH))
  ...
  assert property (p_no_transaction_count_overflow_1)
    ...
    `uvm_error("Prefetch Controller SVA", $sformatf(
               "Overflow condition detected: cnt_q==%0d, DEPTH==%0d, count_up==%0d, count_down==%0d",
               cnt_q,
               DEPTH,
               count_up,
               count_down
  ...
  generate
    if (COREV_PULP) begin : gen_pulp_xpulp_assertions
      ...
      assert property (p_hwlp_end_already_gnt_when_hwlp_branch)
        ...
        `uvm_error("Prefetch Controller SVA", $sformatf(
                   "Hardware Loop End should already be granted"))
      ...
      assert property (p_hwlp_not_used)
        ...
        `uvm_error("Prefetch Controller SVA", $sformatf(
                   "Hardware Loop signals active while COREV_PULP = 0"))
  ...
  assert property (p_branch_implies_req)
    ...
    `uvm_error("Prefetch Controller SVA", $sformatf(
               "Taken branch occurs while fetching is not requested"))
  ...
  assert property (p_branch_invalidates_fifo)
    ...
    `uvm_error("Prefetch Controller SVA", $sformatf(
               "After taken branch the initial FIFO output is accepted"))
  ...
  assert property (p_jump_hwlp_branch_not_together)
    ...
    `uvm_error("Prefetch Controller SVA", $sformatf(
               "hwlp_branch and branch_i happen at the same moment"))
